...
int* buscaLargura_Grafo(Grafo *gr_t, char *nomeCelebridade)
{
    ...
    while(Q_Size(fila) != 0)
    {
        ...
        //Verificar todos os seguidores do vértice atual
...
Vertice* get_seguidor_nao_visitado(Grafo *gr, Vertice *ini)
{
    ...
    //Caso não tenha mais seguidores não visitados
    return NULL;
}
...
int busca_em_profundidade(Grafo *gr, char *nomeProcura)
{
    ...
    do 
    {
        //Retira o primeiro indice da pilha
        S_Pop(&indc, pilha);

        //Pega o vértice referente a ele
        aux = get_vertice_de_indice(gr, indc); 

        if(strcmp(nomeProcura, aux->nomePessoa) == 0 && !flag_inicio)
        {
            //Caso encontre o nome procurado e não é o primeiro loop
            S_Destroy(pilha);
            return contador;
        }
        else
        {
            ...
            if(aux2 != NULL)
            {
                //Encontrou um sesguidor válido
                contador++;
                indc = get_indc_vertice(gr, aux2->nomePessoa);

                //Investigar o seguidor encontrado
                S_Push(&indc, pilha);
            }
            else
            {
                //Se for nulo, chegamos em um "nó sem saída"
                contador--;
            }
    ...
    }while(S_Size(pilha) != 0); //Enquanto a pilha não estiver vazia

    S_Destroy(pilha);

    //Retorna valor inválido caso não tenha encontrado
    return -1;
}
